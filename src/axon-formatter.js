...
import { TrioSrc } from "./axon-parser.js";

export function printAxon(path, options, print) {
  ...
  const nextCharAcrossLines = function (filePos) {
    for (let index = filePos; index < options.originalText.length; ++index) {
      ...
      if (ch == "/" && options.originalText[index + 1] == "*") {
        index += 2;
        while (
          index + 1 < options.originalText.length &&
          !(options.originalText[index] == "*" && options.originalText[index + 1] == "/")
        )
          index++;
  ...
  const parens = function (expr, docs) {
    ...
    if (lParenBreak === pb.hardlineWithoutBreakParent && rParenBreak === pb.hardlineWithoutBreakParent)
      return ["(", pb.indent([lParenBreak, ...docs]), rParenBreak, ")"];
    return ["(", lParenBreak, ...docs, rParenBreak, ")"];
  };

  function argsGroup(args, groupId, argExprs) {
    let parenBreak =
      args.length >= 1 && argExprs[0]._start != null && newlinePrior(argExprs[0]._start.filePos())
    ...
    switch (node._type) {
      ...
      case axon.ExprType.list(): {
        ...
        if (node.vals.length > 1 || trailingComma) {
          return pb.group(
            [
              "[",
              pb.indent([pb.softline, pb.join([",", pb.line], path.map(print, "vals"))]),
              trailingComma ? "," : "",
              pb.softline,
              "]",
            ],
      ...
      case axon.ExprType.dict(): {
        ...
        const pairs = keys.map((k, i) => [
          k,
          values[i] == "" ? "" : [":", pb.ifBreak(" ".repeat(longestKeyLength - k.length + 1), " "), values[i]],
        ]);
        ...
        if (node.vals.length > 1 || trailingComma) {
          return pb.group(
            ["{", pb.indent([pb.softline, pb.join([",", pb.line], pairs)]), trailingComma ? "," : "", pb.softline, "}"],
            { shouldBreak: trailingComma, id: node._group_id },
          );
        }
      ...
      case axon.ExprType.compdef(): {
        ...
        const pairs = [];
        keys.each((k, i) => {
          pairs.push([k, ":", " ".repeat(longestKeyLength - k.length + 1), values[i]]);
        });

        return [
          "defcomp",
          pb.indent([pb.hardline, pb.join(pb.hardline, pairs), pb.hardline, path.call(print, "body")]),
          pb.hardline,
          "end",
        ];
      }
      ...
      case axon.ExprType.call(): {
        ...
        if (
          ...
          !isCallBinaryOpLhs &&
          node.args.length > 0 &&
          node.args[node.args.length - 1]._type == axon.ExprType.func()
        ) {
      ...
      case axon.ExprType.staticCall():
        return [
          path.call(print, "typeRef"),
          ".",
          path.call(print, "funcName"),
          argsGroup(path.map(print, "args"), Symbol(), node.args),
        ];
      ...
      case axon.ExprType.div("/"): {
        ...
        let rOpBreak = newlineAfter(
          options.originalText.indexOf(node._type.op(), node.lhs._end.filePos() + 1) + node._type.op().length - 1,
        )
          ? pb.hardlineWithoutBreakParent
          : " ";
      ...
      case axon.ExprType.dotCall(): {
        ...
        if (node.lhs._type == axon.ExprType.dotCall() && node.lhs.func.name.value != "get") {
          ...
          if (
            path.parent !== null &&
            path.parent._type === axon.ExprType.dotCall() &&
            node._arg_of_dotcall != path.parent
          ) {
            dotBreak = pb.ifBreak(pb.hardlineWithoutBreakParent, pb.softline, { groupId: node._break_if_group });
            dotBreakGroup = dotBreak.groupId;
          }
        ...
        if (
          !node._args_need_parens &&
          !isDotCallBinaryOpLhs &&
          node.args.length > 0 &&
          node.args[node.args.length - 1]._type == axon.ExprType.func()
        ) {
          trailingLamdba = argDocs.splice(-1, 1).pop();
        ...
        if (node.func.name.value == "get" && argDocs.length == 1 && trailingLamdba === null) {
          ...
          const nextLineOpensWithParen =
            argDocs.length === 0 && trailingLamdba === null && nextCharAcrossLines(node._end.filePos() + 1) === "(";
          if (
            argDocs.length > 0 ||
            (trailingLamdba !== null && node.args[node.args.length - 1].params.length != 1) ||
            nextLineOpensWithParen
          ) {
      ...
      case axon.ExprType.ifExpr(): {
        ...
        let lastExprInIf_is_IfWithoutElse =
          ifExprInBlock && endsInIfWithoutElse(node.ifExpr.exprs[node.ifExpr.exprs.length - 1]);
...
function doWrap(docs) {
  return ["do", pb.indent([pb.hardlineWithoutBreakParent, docs]), pb.hardlineWithoutBreakParent, "end"];
}
...
export function printTrio(path, options, print) {
  const node = path.getNode();
