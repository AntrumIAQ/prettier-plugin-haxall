...
/**
 * ActorContext is standardized thread local state stored in by
 * "cx" key
 */
export abstract class ActorContext extends sys.Obj {
  static type$: sys.Type
  /**
   * Actor.locals key "cx" for context state
   */
  static actorLocalsKey(): string;
}
 ...
 * FutureTest
 */
export class FutureTest extends sys.Test {
  static type$: sys.Type
  pool(): ActorPool;
  pool(it: ActorPool): void;
  completeLaterOk(f: Future, val: sys.JsObj | null): void;
  verifyWrap(f: Future, w: Future): void;
  callLater(f: ((arg0: sys.JsObj) => void)): void;
  teardown(): void;
  static make(...args: unknown[]): FutureTest;
  completeLaterErr(f: Future, err: sys.Err): void;
  testThen(): void;
  completeLaterCancel(f: Future): void;
  testSubclass(): void;
  testCompletable(): void;
}
...
/**
 * Raised when attempting to access results of a future in the
 * pending state.
 */
export class NotCompleteErr extends sys.Err {
  static type$: sys.Type
  /**
   * Construct with specified error message and optional root
   * cause.
   */
  static make(msg?: string, cause?: sys.Err | null, ...args: unknown[]): NotCompleteErr;
}
...
export class Future extends sys.Obj {
  ...
  cancel(): void;
  /**
   * Return the exception raised by the asynchronous computation
   * or null if the future completed successfully.  This method
   * can only be used after completion, otherwise if status
   * pending then raise NotCompleteErr.
   */
  err(): sys.Err | null;
  /**
  ...
  /**
   * Get JavaScript Promise object which backs this Future. Only
   * available in JavaScript environments.
   */
  promise(): sys.JsObj;
  /**
   * Register a callback function when this future completes in
   * either the ok or err/cancel state.  Return a new future that
   * may be chained for additional async operations that will
   * return the result of the given callback.
   * 
   * In the Java VM this operation is a blocking operation that
   * has the same effect as calling {@link waitFor | waitFor} and
   * then invoking the given callback with the result of {@link get | get}
   * or {@link err | err}.
   * 
   * In JavaScript this operation wraps Promise.then with the
   * same semantics.
   */
  then(onOk: ((arg0: sys.JsObj | null) => sys.JsObj | null), onErr?: ((arg0: sys.Err) => sys.JsObj | null) | null): this;
  /**
