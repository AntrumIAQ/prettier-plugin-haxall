...
export class Str extends Obj {
  ...
  /**
   * Split string into lines and call specified function with
   * each line. Each line is terminated by \n, \r\n, \r, or EOF
   * using the same semantics as {@link splitLines | splitLines}
   */
  static eachLine(self: string, f: ((arg0: string) => void)): void;
  /**
...
export class StrBuf extends Obj {
  ...
  capacity(it: number): void;
  /**
   * Call {@link join | join} but only if x is non-null.
   */
  joinNotNull(x: JsObj | null, sep?: string): this;
  /**
  ...
  /**
   * Remove any trailing {@link Str.trim | whitespace} in the
   * buffer.  Return this.
   */
  trimEnd(): this;
  /**
  ...
  clear(): this;
  /**
   * Add string with its leading and trailing {@link Str.trim | whitespace}
   * trimmed.  Return this.
   */
  addTrim(x: string): this;
  /**
...
export class File extends Obj {
  ...
  /**
   * Read the file's input stream using a callback. The input
   * stream is guaranteed to be closed when the callback exits.
   * 
   * The method returns the value returned by the callback.
   */
  withIn(f: ((arg0: InStream) => JsObj | null)): JsObj | null;
  /**
   * Default implementation is convenience for {@link Uri.mimeType | uri.mimeType}.
   */
  mimeType(): MimeType | null;
  /**
  ...
  uri(): Uri;
  /**
   * Write to the file's output stream using a callback. The
   * output stream is guaranteed to be closed when the callback
   * exits.
   */
  withOut(f: ((arg0: OutStream) => void)): void;
  /**
...
export class Env extends Obj {
  ...
  parent(): Env | null;
  /**
   * Get the values for this key organized by each pod that
   * defines the key. The keys of the map are the pod names, and
   * the values of the map are the key values defined by that
   * pod.
   */
  indexByPodName(key: string): Map<string, List<string>>;
  /**
   ...
   *   to use for logging the compilation process and errors
   * - logOut: an output stream to capture logging
   * - output: The output format of the compile javascript. Use
   *   "legacy" for old js format, "js" for CommonJs and  "esm" for
   *   ES module. The default is "js" if not specified.
   */
