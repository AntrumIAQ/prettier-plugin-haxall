...
export class JsonInStream extends sys.InStream {
  static type$: sys.Type
  /**
   * Override this method to transform each Str:Obj? into an
   * arbitrary Obj during parsing.
   */
  transformObj(obj: sys.Map<string, sys.JsObj | null>): sys.JsObj;
  /**
...
/**
 * SyntheticFile provides default no-op implementation of File
 * API
 */
export class SyntheticFile extends sys.File {
  static type$: sys.Type
  /**
   * No-op
   */
  modified(): sys.DateTime | null;
  modified(it: sys.DateTime | null): void;
  /**
   * Return null
   */
  parent(): sys.File | null;
  /**
   * Return this
   */
  normalize(): sys.File;
   ...
   * Raise IOErr
   */
  in(bufferSize?: number | null): sys.InStream;
  /**
   * Return empty list
   */
  list(pattern?: sys.Regex | null): sys.List<sys.File>;
  /**
   * Return another instance of SyntheticFile
   */
  plus(uri: sys.Uri, checkSlash?: boolean): sys.File;
  /**
   * Return null
   */
  size(): number | null;
   ...
   * Raise IOErr
   */
  out(append?: boolean, bufferSize?: number | null): sys.OutStream;
  /**
   * Constructor
   */
  static make(uri: sys.Uri, ...args: unknown[]): SyntheticFile;
  /**
   * Raise IOErr
   */
  deleteOnExit(): sys.File;
  /**
   * Return false
   */
  exists(): boolean;
   ...
   * Raise IOErr
   */
  mmap(mode?: string, pos?: number, size?: number | null): sys.Buf;
  /**
   * Raise IOErr
   */
  open(mode?: string): sys.Buf;
  /**
   * Raise IOErr
   */
  moveTo(to: sys.File): sys.File;
}
