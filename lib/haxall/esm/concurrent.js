...
class ConcurrentMapTest extends sys.Test {
  ...
  test() {
    ...
    let count = 0;
    let r = m.eachWhile((x) => {
      ((this$) => { let $_u25 = count;count = sys.Int.increment(count); return $_u25; })(this$);
...
class ActorTest extends sys.Test {
  ...
  static messaging(msg) {
    const this$ = this;
    let $_u27 = msg;
    if (sys.ObjUtil.equals($_u27, "const")) {
      return ActorTest.constObj();
    }
    else if (sys.ObjUtil.equals($_u27, "serial")) {
      return sys.ObjUtil.coerce(sys.ObjUtil.with(SerMsg.make(), (it) => {
        it.i(123);
        return;
      }), SerMsg.type$);
    }
    else if (sys.ObjUtil.equals($_u27, "throw")) {
      throw sys.UnknownServiceErr.make();
    }
    else if (sys.ObjUtil.equals($_u27, "mutable")) {
    ...
    futures.each((f,i) => {
      if (sys.ObjUtil.equals(f.status(), FutureStatus.cancelled())) {
        ...
        try {
          ...
          if ($_u28 instanceof sys.InterruptedErr) {
            let e = $_u28;
            ;
